...
CREATE OR ALTER PROCEDURE Calculate_Alliant_Stop_Times @branch VARCHAR(10), @date1 DATE, @date2 DATE, @routes VARCHAR(50)
AS
BEGIN
    SELECT
	ricustmr.cu_id,
	...
	CASE WHEN 
		ricustmr.cu_route = LAG(ricustmr.cu_route) 
							OVER (ORDER BY cu_route,rainvhhd.hh_starttime) 
		THEN CAST(
				CAST(
					CAST(rainvhhd.hh_starttime as datetime) - 
					CAST(
						LAG(rainvhhd.hh_stoptime) 
						OVER (ORDER BY cu_route, rainvhhd.hh_starttime) 
					as datetime) 
				AS Time) 
			AS VARCHAR(50)) 
		ELSE ' ' 
	END [Time Between Stops]

INTO [Reroute].[dbo].[tempStopTimes]

FROM            
	[Alliant].[DBO].rainvhhd INNER JOIN
...
AND 
	cu_branch = @branch
AND
    cu_route IN (SELECT TRIM(value) FROM STRING_SPLIT(@routes, ','))
AND 
	hh_date BETWEEN @date1 AND @date2 -- The begin and end dates of the most recent time study 
ORDER BY
